...
from pipeline.create_dataset import dataset
...
def build_vocab_dialogs(cfg, directory):
  ...
  path = os.path.join(directory, 'dialogs_dialogs.txt')
  with open(path, 'w', encoding='utf-8') as f:
...
def post_filter(cfg, directory=directory=os.path.join('data', 'filtered')):
