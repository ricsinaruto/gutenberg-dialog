...
def run():
    ...
    parser.add_argument("--in_file", type=str, default="responses/gutenberg_de/test_inputs.txt")
    parser.add_argument("--out_file", type=str, default="responses/gutenberg_de/gutenberg_valmin.txt")
    ...
    parser.add_argument("--model_checkpoint", type=str, default="./runs/gutenberg_de/", help="Path, url or short name of the model")
    ...
    if args.interact:
        ...
        with open(args.out_file, 'w') as f:
            for i, sample in enumerate(history):
                inputs = [tokenizer.encode(raw_text) for raw_text in sample]
                with torch.no_grad():
                    out_ids = sample_sequence(inputs, tokenizer, model, args)
                out_text = tokenizer.decode(out_ids, skip_special_tokens=True)
                #print(out_text)
                f.write(processed(out_text) + '\n')

                if not i % 100:
                    print(i / len(history) * 100)
